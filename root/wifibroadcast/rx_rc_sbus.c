...
void open_and_configure_interface(const char *name, monitor_interface_t *interface) 
{
	...
	if (nLinkEncap == DLT_IEEE802_11_RADIO) {
		sprintf(pcap_filter_char, "ether[0x00:4] == 0xb4bf0000");
	} else {
...
void process_packet(monitor_interface_t *interface, int serialport) 
{
	...
	if (pu8Payload[1] != 0xbf || pu8Payload[0] != 0xb4) {
		return;
	}
	...
	// Process packet
	// Note: Now pu8Payload is at "framedata.crc"
	...
	pu8Payload += 1;
	// verify crc32
	uint32_t crc_magic = htonl(0x11451419);
...
{
	monitor_interface_t wifi_iface;
	...
	// check the type of Wi-Fi nic driver
	char path[45], line[100];
	FILE* procfile;
	snprintf(path, 45, "/sys/class/net/%s/device/uevent", iniparser_getstring(ini, "rx_rc_sbus:nic", NULL));
	...
	if(strncmp(line, "DRIVER=ath9k", 12) == 0) { 
		fprintf(stderr, "rx_rc_sbus: Driver: Atheros\n");
	} else { 
		fprintf(stderr, "rx_rc_sbus: Driver: Ralink\n");
	}
	open_and_configure_interface(iniparser_getstring(ini, "rx_rc_sbus:nic", NULL), &wifi_iface);
	fclose(procfile);
	...
	uartfd = open( iniparser_getstring(ini, "rx_rc_sbus:uart", NULL),
					O_RDWR|O_NOCTTY|O_NDELAY);			
	if (uartfd < 0) {
		fprintf(stderr, "rx_rc_sbus ERROR: Unable to open UART. Ensure it is not in use by another application\n");
	}
	...
	for(;;) {
	    ...
	    fd_set readset;
	    FD_ZERO(&readset);
		FD_SET(wifi_iface.selectable_fd, &readset);
			
	    int n = select(wifi_iface.selectable_fd+1, &readset, NULL, NULL, &to); 
	    if (n == 0) 
			continue;
		if (FD_ISSET(wifi_iface.selectable_fd, &readset)) {
			process_packet(&wifi_iface, uartfd);
		}
	}
	close(uartfd);
	close(wifi_iface.selectable_fd);
